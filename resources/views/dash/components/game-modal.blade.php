...
<div x-show="$store.gameModal.show" style="display:none" @keydown.escape.window="$store.gameModal.show = false"
    ...
    <div @click.outside="$store.gameModal.show = false" class="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 flex flex-col max-h-[95vh]">
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between shrink-0">
            <h2 class="text-lg font-bold text-slate-800 dark:text-white" x-text="!$store.gameModal.canModifyMatches() ? '{{ __('matches.details') }}' : ($store.gameModal.editingId ? '{{ __('matches.modal.edit_title') }}' : '{{ __('matches.modal.add_title') }}')"></h2>
        ...
        <form @submit.prevent="$store.gameModal.save()" class="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 shadow-sm block">{{ __('matches.modal.opponent') }}</label>
                    <input type="text" x-model="$store.gameModal.formData.opponent_name" required placeholder="{{ __('matches.modal.opponent_placeholder') }}" :readonly="!$store.gameModal.canModifyMatches()"
                ...
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 shadow-sm block">{{ __('matches.modal.location') }}</label>
                    <input type="text" x-model="$store.gameModal.formData.location" required placeholder="{{ __('matches.modal.location_placeholder') }}" :readonly="!$store.gameModal.canModifyMatches()"
                           class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary shadow-sm disabled:opacity-70">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 shadow-sm block">{{ __('matches.modal.match_date') }}</label>
                ...
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 shadow-sm block">{{ __('matches.modal.team') }}</label>
                    <select x-model="$store.gameModal.formData.squad_id" @change="$store.gameModal.fetchMembers($event.target.value)" required :disabled="!$store.gameModal.canModifyMatches()"
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary shadow-sm disabled:opacity-70">
                        <option value="">{{ __('matches.modal.select_team') }}</option>
                ...
                <div class="bg-indigo-50/50 dark:bg-indigo-900/10 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-900/30 space-y-4">
                    <template x-for="i in [1,2,3,4,5]" :key="i">
                        <div class="flex items-center gap-4">
                            <span class="text-xs font-bold text-indigo-400 w-10 text-center" x-text="`{{ __('matches.modal.set', ['num' => '']) }}`.replace(':num', i)"></span>
                            <div class="flex-1 grid grid-cols-2 gap-3 items-center">
                                <div class="relative">
                                    <input type="number" x-model="$store.gameModal.formData['set'+i+'_home']" placeholder="{{ __('matches.modal.home') }}" :readonly="!$store.gameModal.canModifyMatches()"
                                    ...
                                    <div class="absolute left-2 top-1/2 -translate-y-1/2 text-[7px] font-black text-slate-300 uppercase leading-none">{{ __('matches.modal.home_short') }}</div>
                                </div>
                                <div class="relative">
                                    <input type="number" x-model="$store.gameModal.formData['set'+i+'_away']" placeholder="{{ __('matches.modal.away') }}" :readonly="!$store.gameModal.canModifyMatches()"
                                           class="w-full pl-8 pr-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-center font-bold outline-none focus:ring-2 focus:ring-primary shadow-sm disabled:opacity-70">
                                    <div class="absolute left-2 top-1/2 -translate-y-1/2 text-[7px] font-black text-slate-300 uppercase leading-none">{{ __('matches.modal.away_short') }}</div>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            {{-- Notes --}}
            <div>
                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 shadow-sm block">{{ __('matches.modal.notes') }}</label>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" :disabled="$store.gameModal.saving" x-show="$store.gameModal.canModifyMatches()"
                    ...
                    <span x-text="$store.gameModal.editingId ? '{{ __('admin.save') }}' : '{{ __('admin.add') }}'"></span>
                </button>
            </div>
        </form>
    </div>
