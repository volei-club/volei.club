            ...
            <div x-show="currentPage.startsWith('/dash/grupe')" x-data="teamManager()" class="h-full flex flex-col relative">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('admin.teams.title') }}</h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('admin.teams.subtitle') }}</p>
                    </div>
                ...
                <div class="mb-6 flex flex-col md:flex-row gap-4 items-center">
                    <div class="w-full md:flex-1 relative">
                        ...
                        <input 
                            type="text" 
                            x-model="search" 
                            @input.debounce.500ms="fetchTeams()" 
                    ...
                    <template x-if="user?.role === 'administrator'">
                        <div class="w-full md:w-64">
                            <select id="teamFilterClub" x-model="filters.club_id" @change="fetchTeams(); updateHash()" class="w-full px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all appearance-none cursor-pointer text-sm">
                                <option value="">{{ __('admin.teams.all_clubs_filter') }}</option>
                ...
                <div x-show="loading" style="display:none" class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm rounded-2xl">
                    ...
                    <p class="text-slate-500 font-medium">{{ __('admin.teams.loading') }}</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[600px]">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 text-slate-500 uppercase text-xs tracking-wider">
                                <th class="px-6 py-4 font-bold">{{ __('admin.teams.name_column') }}</th>
                                <template x-if="user?.role === 'administrator'">
                                    <th class="px-6 py-4 font-bold">{{ __('admin.teams.club_column') }}</th>
                                </template>
                                <th class="px-6 py-4 font-bold text-right">{{ __('admin.teams.actions_column') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-sm">
                            <template x-for="team in teams" :key="team.id">
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <template x-if="user?.role === 'administrator'">
                                        <td class="px-6 py-4">
                                            <span class="px-2 py-1 bg-slate-100/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border border-slate-200/50 dark:border-slate-700/50 rounded-lg text-[11px] font-bold uppercase tracking-wide inline-flex items-center" x-text="availableClubs.find(c => c.id === team.club_id)?.name || `{{ __('admin.unknown') }}`"></span>
                                        </td>
                                    </template>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <button @click="openPreview(team)" class="p-2 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors" title="{{ __('admin.squads.preview_title') }}">
                                                <span class="material-symbols-outlined text-[20px]">groups</span>
                                            </button>
                                            <button @click="openModal(team)" class="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors" title="{{ __('admin.edit') }}">
                                                <span class="material-symbols-outlined text-[20px]">edit</span>
                                            </button>
                                            <button @click="deleteTeam(team.id)" class="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="{{ __('admin.delete') }}">
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm">
                            ...
                            <template x-if="user?.role === 'administrator'">
                                <div class="flex items-center text-sm font-semibold text-slate-500 mt-2">
                                    ...
                                    <span class="px-2 py-1 bg-slate-100/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border border-slate-200/50 dark:border-slate-700/50 rounded-lg text-[11px] font-bold uppercase tracking-wide" x-text="availableClubs.find(c => c.id === team.club_id)?.name || `{{ __('admin.unknown') }}`"></span>
                                </div>
                            </template>
                            <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex gap-2">
                                <button @click="openPreview(team)" class="flex-1 flex items-center justify-center gap-2 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 rounded-xl font-bold text-sm transition-colors">
                ...
                <div x-show="teams.length === 0 && !loading" class="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 border-dashed">
                    ...
                    <p class="text-slate-500">{{ __('admin.teams.empty_state') }}</p>
                </div>
                ...
                <div x-show="showPreview" style="display:none" @keydown.escape.window="showPreview = false" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
                    <div class="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col max-h-[85vh]">
                        ...
                        <div class="p-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between shrink-0">
                            <div>
                                <h3 class="text-lg font-bold text-slate-900 dark:text-white" x-text="previewTeam?.name"></h3>
                                <p class="text-sm text-slate-500" x-text="`{{ __('admin.squads.members_count', ['count' => '']) }}`.replace(':count', (previewTeam?.users?.length || 0))"></p>
                            </div>
                        ...
                        <div class="overflow-y-auto flex-1">
                            <template x-if="previewTeam?.users?.length === 0">
                                <div class="py-16 text-center">
                                    <span class="material-symbols-outlined text-5xl text-slate-300 dark:text-slate-600 mb-3">group_off</span>
                                    <p class="text-slate-500 text-sm">{{ __('admin.teams.empty_state') }}</p>
                                </div>
                            </template>
                            <template x-for="member in previewTeam?.users" :key="member.id">
                ...
                <div x-show="showModal" style="display: none;" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
                    <div class="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col max-h-[90vh]">
                        <div class="p-6 border-b border-slate-100 dark:border-slate-700 shrink-0">
                            <h3 class="text-xl font-bold" x-text="form.id ? `{{ __('admin.teams.edit_title') }}` : `{{ __('admin.teams.add_title') }}`"></h3>
                        </div>
                        <form @submit.prevent="saveTeam()" class="flex flex-col overflow-hidden">
                            <div class="p-6 overflow-y-auto">
                            <div class="mb-4">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">{{ __('admin.teams.name_label') }}</label>
                                <input x-model="form.name" type="text" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary outline-none text-slate-900 dark:text-white transition-all" placeholder="{{ __('admin.teams.name_placeholder') }}"/>
                            </div>
                            
                            <template x-if="user?.role === 'administrator'">
                                <div class="mb-4">
                                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">{{ __('admin.teams.belongs_to_club_label') }}</label>
                                    <select x-model="form.club_id" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all appearance-none cursor-pointer">
                                        <option value="" disabled selected>{{ __('admin.teams.choose_club') }}</option>
                                        <template x-for="c in availableClubs" :key="c.id">
                                            <option :value="c.id" x-text="c.name"></option>
                                        </template>
                                    </select>
