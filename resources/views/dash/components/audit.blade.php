<div x-show="currentPage === '/dash/audit'" x-data="auditManager()" class="h-full flex flex-col relative">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
        <div>
            <h3 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('audit.title') }}</h3>
    ...
    <div x-show="loading" style="display:none" class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm rounded-2xl">
        ...
        <p class="text-slate-500 font-medium">{{ __('audit.loading') }}</p>
    </div>
    ...
    <div class="mb-6 flex flex-wrap gap-4">
        <div class="w-full md:w-48">
            <select x-model="filters.event" @change="fetchLogs()" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm appearance-none cursor-pointer">
                ...
                <option value="created">{{ __('audit.filters.created') }}</option>
                <option value="updated">{{ __('audit.filters.updated') }}</option>
                <option value="deleted">{{ __('audit.filters.deleted') }}</option>
            </select>
        </div>
        <div class="w-full md:w-48">
            <select x-model="filters.type" @change="fetchLogs()" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm appearance-none cursor-pointer">
                ...
                <option value="User">{{ __('audit.filters.users') }}</option>
                <option value="Subscription">{{ __('audit.filters.subscriptions') }}</option>
                <option value="Club">{{ __('audit.filters.clubs') }}</option>
                <option value="Team">{{ __('audit.filters.teams') }}</option>
                <option value="Squad">{{ __('audit.filters.squads') }}</option>
            </select>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden flex-1 flex flex-col">
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 text-slate-500 uppercase text-xs tracking-wider">
                        <th class="px-6 py-4 font-bold">{{ __('audit.table.user') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('audit.table.action') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('audit.table.entity') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('audit.table.modifications') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('audit.table.date_ip') }}</th>
                    </tr>
                </thead>
                    ...
                    <template x-for="log in logs" :key="log.id">
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                <template x-if="!log.user">
                                    <span class="text-slate-400 italic">{{ __('audit.table.system_anon') }}</span>
                                </template>
                            </td>
                            <td class="px-6 py-4">
                                <span :class="{
                                ...
                                }" class="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-sm" x-text="log.event === 'created' ? '{{ __('audit.events.created') }}' : (log.event === 'updated' ? '{{ __('audit.events.updated') }}' : '{{ __('audit.events.deleted') }}')"></span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-semibold text-slate-700 dark:text-slate-300" x-text="translateAuditType(log.auditable_type)"></div>
                                ...
                                <div class="max-w-xs overflow-hidden">
                                    ...
                                    <template x-if="log.event === 'created'">
                                        <button @click="openLogDetails(log)" class="text-xs text-primary hover:text-primary-dark font-bold flex items-center gap-1.5 transition-colors group/link">
                                            ...
                                            <span>{{ __('audit.table.view_initial') }}</span>
                                        </button>
                                    </template>
                                    <template x-if="log.event === 'deleted'">
                                        <button @click="openLogDetails(log)" class="text-xs text-red-500 hover:text-red-600 font-bold flex items-center gap-1.5 transition-colors group/link">
                                            ...
                                            <span>{{ __('audit.table.view_deleted') }}</span>
                                        </button>
                                    </template>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-slate-900 dark:text-white font-medium" x-text="new Date(log.created_at).toLocaleString(locale)"></div>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl p-5 shadow-sm space-y-4">
                    <div class="flex justify-between items-start border-b border-slate-50 dark:border-slate-800 pb-3">
                        <div class="flex-1">
                           ...
                           <template x-if="!log.user">
                               <span class="text-slate-400 italic text-xs">{{ __('audit.table.system_anon') }}</span>
                           </template>
                        </div>
                        <span :class="{
                            'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': log.event === 'created',
                            'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400': log.event === 'updated',
                            'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400': log.event === 'deleted'
                        }" class="px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider shrink-0" x-text="log.event === 'created' ? '{{ __('audit.events.created') }}' : (log.event === 'updated' ? '{{ __('audit.events.updated') }}' : '{{ __('audit.events.deleted') }}')"></span>
                    </div>

                    <div class="grid grid-cols-2 gap-4 text-xs">
                        <div>
                            <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">{{ __('audit.table.entity') }}</p>
                            <div class="font-semibold text-slate-700 dark:text-slate-300" x-text="translateAuditType(log.auditable_type)"></div>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">{{ __('audit.table.date_ip') }}</p>
                            <div class="text-slate-600 dark:text-slate-400 font-medium" x-text="new Date(log.created_at).toLocaleString(locale)"></div>
                        </div>
                    </div>

                    <div class="space-y-2">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">{{ __('audit.table.modifications') }}</p>
                        <template x-if="log.event === 'updated'">
                        ...
                        <template x-if="log.event === 'created' || log.event === 'deleted'">
                            <button @click="openLogDetails(log)" class="w-full py-2 bg-slate-50 dark:bg-slate-800 text-xs font-bold text-primary rounded-xl flex items-center justify-center gap-2">
                                ...
                                <span x-text="log.event === 'created' ? '{{ __('audit.table.view_initial') }}' : '{{ __('audit.table.view_deleted') }}'"></span>
                            </button>
                        </template>
                    </div>
                </div>
        ...
        <div class="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between shrink-0">
            <span class="text-xs text-slate-500">
                {{ __('admin.pagination_info', ['current' => '<span class="font-bold" x-text="pagination.current_page"></span>', 'last' => '<span class="font-bold" x-text="pagination.last_page"></span>']) }}
            </span>
            <div class="flex gap-2">
                <button @click="changePage(pagination.current_page - 1)" :disabled="pagination.current_page <= 1" class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-bold hover:bg-slate-50 disabled:opacity-50 transition-all">{{ __('admin.previous') }}</button>
                <button @click="changePage(pagination.current_page + 1)" :disabled="pagination.current_page >= pagination.last_page" class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-bold hover:bg-slate-50 disabled:opacity-50 transition-all">{{ __('admin.next') }}</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showDetailsModal" style="display: none;" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col max-h-[85vh]">
            <div class="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                <h3 class="text-xl font-bold">{{ __('audit.modal.details_title') }}</h3>
            ...
            <div class="p-6 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                <button @click="showDetailsModal = false" class="px-6 py-2 bg-primary text-white rounded-xl font-bold">{{ __('audit.modal.close') }}</button>
            </div>
        </div>
    </div>
</div>
