                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">
                            Bun venit, <span x-text="user?.name?.split(` `)[0]"></span>! 👋
                        </h2>
                        <p class="text-sm text-slate-500 mt-0.5" x-text="new Date().toLocaleDateString(`ro-RO`, { weekday: `long`, year: `numeric`, month: `long`, day: `numeric` })"></p>
                    </div>
                    ...
                    <section :class="['administrator', 'manager', 'antrenor'].includes(user?.role) ? 'xl:col-span-6' : 'xl:col-span-12'" class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">Conversații active</p>
                            </div>
                            <button @click="navigate(`/dash/mesaje`)" class="flex items-center gap-1 text-xs font-bold text-primary hover:underline">
                                Vezi Toate <span class="material-symbols-outlined text-[16px]">arrow_forward</span>
                            </button>
                        </div>
                        <div class="space-y-2">
                            ...
                            <template x-for="conv in stats.recent_conversations" :key="conv.conversation_id">
                                <div @click="navigate(`/dash/mesaje`)" class="group flex gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors cursor-pointer border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-slate-500 truncate" x-text="conv.last_message || `Niciun mesaj`"></p>
                                    </div>
                                </div>
                            </template>
                        </div>
