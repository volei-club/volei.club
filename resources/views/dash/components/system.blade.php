<div x-show="currentPage === '/dash/sistem'" class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('system.title') }}</h1>
            <p class="text-slate-500 text-sm">{{ __('system.subtitle') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" x-data="systemManager()">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700/50">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h3 class="font-bold text-slate-900 dark:text-white">{{ __('system.export.title') }}</h3>
                    <p class="text-xs text-slate-500">{{ __('system.export.subtitle') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-3">
                <button @click="exportData('all')" class="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 hover:bg-primary hover:text-white rounded-2xl transition-all group border border-transparent hover:border-primary/20">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-left">
                            <div class="text-sm font-bold">{{ __('system.export.all.title') }}</div>
                            <div class="text-[10px] opacity-60">{{ __('system.export.all.desc') }}</div>
                        </div>
                    </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700/50">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h3 class="font-bold text-slate-900 dark:text-white">{{ __('system.import.title') }}</h3>
                    <p class="text-xs text-slate-500">{{ __('system.import.subtitle') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="relative group">
                    ...
                    <div class="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-8 flex flex-col items-center justify-center transition-colors group-hover:border-primary/50 bg-slate-50/50 dark:bg-slate-900/30">
                        ...
                        <div class="text-sm font-medium text-slate-600 dark:text-slate-400" x-text="selectedFile ? selectedFile.name : `{{ __('system.import.dropzone.placeholder') }}`"></div>
                        <div class="text-[10px] text-slate-400 mt-1" x-show="!selectedFile">{{ __('system.import.dropzone.max_size') }}</div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-800/20">
                    ...
                    <p class="text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed italic">
                        <strong>{{ __('system.import.warning.label') }}</strong> {!! __('system.import.warning.text') !!}
                    </p>
                </div>
                ...
                <button @click="importData()" 
                    ...
                    <span x-text="importing ? `{{ __('system.import.importing') }}` : `{{ __('system.import.button') }}`"></span>
                </button>
            </div>
        </div>
    </div>
